...
public:
  ...
  bool is_dead() { return health <= 0 ? true : false; }
};
...
int main() {
  ...
  Player *ross = new Player(); // ross is PONINTER TO PLAYER OBJECT. NOTE: NOT A
                               // PLAYER OBJECT.
  ...
  (*sony).name = "Sony"; // Here, () has higher precedence. So, first we
                         // dereference the object and we accessing the members
                         // stored in that object-pointer's holds address.
  ...
  // clearing the space allocated in the heap. Some compiler will automatically
  // do this.
  delete sony;
  delete ross;
